...
- LLM/RAG substituível: mock, OpenAI, Azure OpenAI, Groq ou Gemini.
- Executor de dados: mock, SQL Server, Databricks ou PostgreSQL.
...
cd frontend-angular
npm install
npm start   # proxy para http://localhost:8080
```
...
```bash
cp deploy/env/.env.local deploy/env/.env   # opcional
docker compose -f deploy/docker/docker-compose.yml up --build
```

Templates de ambiente: `deploy/env/.env.hom.example`, `deploy/env/.env.prod.example`.
